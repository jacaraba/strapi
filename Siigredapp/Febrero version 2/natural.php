<?php
	...
	*/function natural_before_insert(&$data, $memberInfo, &$args) {
    $table = 'natural';
    $vcrCodigoField = 'VcrCodForm';
    $prefix = '41630101424-01-';

    // Obtener el último consecutivo de la tabla
    $lastId = sqlValue("SELECT MAX(id) FROM {$table}");
    $lastVcrCodigo = sqlValue("SELECT $vcrCodigoField FROM {$table} WHERE id={$lastId}");

    // Obtener el número de dígitos del consecutivo actual
    $numDigits = strlen(str_replace($prefix, '', $lastVcrCodigo));
	
    // Generar el nuevo consecutivo
    $newConsecutivo = $lastId ? intval(str_replace($prefix, '', $lastVcrCodigo)) + 1 : 340;
    $newVcrCodigo = $prefix . str_pad($newConsecutivo, $numDigits, '0', STR_PAD_LEFT);
   

    // Actualizar el valor del campo VcrCodForm
    $data[$vcrCodigoField] = $newVcrCodigo;
    return TRUE;
}
